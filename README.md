...
实现了判定胜负、悔棋、撤销悔棋和人机对战，目前人机的棋力还比较弱。。
...
目前我了解到的最靠谱的思路就是： 极大极小值搜索算法 + Alpha-Beta剪枝 + 启发式搜索函数。

人机的代码单独放在名为**machine.js** 的文件里，人机和棋盘的代码是分离的，因为计算量很大，便于使用web worker。

就目前的实现而言，棋力还比较弱，我让电脑走的后手，对局势的评分采取相同连子数，黑棋的分比白棋的分多10倍，所以现在的情况就是，电脑会不停的堵人，但是它不会取胜。
